...
const getListSeats = async (req, res) => {
  try {
    ...
    const querySql = `
        ...
        where showtimes.id = seats.showtimeId AND showtimes.startTime = :startTime
...
const bookingSeats = async (req, res) => {
  try {
    const { id } = req.params;
    await Seat.update({ status: false }, { where: { id, status: true } });
    res.status(200).send({ messages: "Booked Seat" });
  } catch (error) {
    res.status(500).send(error);
  }
};
